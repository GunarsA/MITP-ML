...
class LayerSwish:
    ...
    def forward(self, x: Variable):
        self.x = x
        self.output = Variable(self.x.value / (1 + np.exp(-self.x.value)))
        return self.output

    def backward(self):
        self.x.grad += (self.output.value + (1.0 / (1.0 + np.exp(-self.x.value))) * (1 - self.output.value))\
                       * self.output.grad
...
class LossMSE:
    ...
    def backward(self):
        self.y_prim.grad += -2.0 * (self.y.value - self.y_prim.value)
...
def r2_score(y: Variable, y_prim: Variable):
    return 1 - np.sum((y.value - y_prim.value) ** 2) / np.sum((y.value - np.mean(y.value)) ** 2)
...
for epoch in range(1, 1000):
    ...
    for dataloader in [dataloader_train, dataloader_test]:
        ...
        for x, y in dataloader:
            ...
            r2 = r2_score(y=Variable(value=y), y_prim=y_prim)
    ...
    if epoch % 100 == 0:
        ...
        ax4 = ax1.twinx()
        ax4.plot(r2_plot_test, 'b-', label='r2')
        ax1.legend()
